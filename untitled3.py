...
df = pd.read_csv("../data/afin20170930.csv",header=0,dtype={'SEC_IND_CODE':"str"})

#预处理：删除行业空值行，根据decribe，代替不合理极值
#df['SEC_IND_CODE']=[ str(a)[0:4] for a in df['SEC_IND_CODE'] ]            ####这是处理列值的一种方法
df = df[df['SEC_IND_CODE']!='nan']


df['SEC_IND_CODE']=df['SEC_IND_CODE'].apply(lambda x:str(x)[0:4])             ##使用一级行业
...
num_df = df.values[:,2:]                      #dataframe向numpy转化，去除索引S_INFO_WINDCODE及SEC_IND_CODE
...
grouped = handle_df1.groupby(handle_df1['SEC_IND_CODE'])     
...
grouped1 = handle_df.groupby(handle_df['SEC_IND_CODE'])
...
stock_df = pd.merge(handle_df,df_ind,left_on='SEC_IND_CODE',right_index=True)               #注意连接方法，左用字段，右使用索引
stock_df_all = pd.merge(stock_df,df_ind_count,left_on='SEC_IND_CODE',right_index=True)
...
def getnum(x):
    indid = list(stock_df_all[stock_df_all['S_INFO_WINDCODE']==x]['SEC_IND_CODE'])[0]          #取得 公司x对应行业 ,注意有可能多个
    s_value =  list(stock_df_all[stock_df_all['S_INFO_WINDCODE']==x][s])[0]            #取得 公司x对应指标s,注意有可能多个
    
    li = list(stock_df_all[stock_df_all['SEC_IND_CODE']==indid][s].sort_values(ascending=False))           ##取得对应行业公司的值，并排序
...
stock_df_all.rename(columns={'SEC_IND_CODE':'str_SEC_IND_CODE'},inplace=True)
fortest = estools(host="10.237.2.132",index="airr_2017.12.21",doc_type="airr_mapping")
fortest.insertDataframeIntoElastic(stock_df_all,chunk_size = 2000)


##df2 = df.dropna(axis=0, how='all')
#grouped = df.groupby(df['SEC_IND_CODE'])
#
